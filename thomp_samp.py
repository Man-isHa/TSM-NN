...
G_loss = tf.reduce_mean(tf.reduce_sum(x*tf.log(x/G_z),axis=1)) + tf.reduce_mean(tf.reduce_sum(G_z*tf.log(G_z/x),axis=1)) 
...
with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
    G_optim = tf.train.AdamOptimizer(lr, beta1=0.5).minimize(G_loss, var_list=T_vars)
...
for epoch in range(train_epoch):
    ...
    print('[%d/%d] - ptime: %.2f ,loss_g: %.3f' % ((epoch + 1), train_epoch, per_epoch_ptime, np.mean(G_losses)))
...
plt.savefig('Generated_dist_kl.png')
